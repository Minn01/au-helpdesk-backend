...
export const validateAttachmentFile = (file: Express.Multer.File) => {
  if (file.size > MAX_ATTACHMENT_BYTES)
    throw new HttpError(413, "FILE_TOO_LARGE", "Each attachment must be 13 MB or smaller");
  if (!allowedTypes.has(file.mimetype))
    throw new HttpError(415, "UNSUPPORTED_FILE_TYPE", "This attachment type is not allowed");
