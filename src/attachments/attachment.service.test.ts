...
const textFile = (
  name = "notes.txt",
  body = Buffer.from("printer makes a grinding noise"),
): Express.Multer.File => ({
  fieldname: "files",
  originalname: name,
  encoding: "7bit",
...
const harness = (
  ...
  const attachment =
    options.attachment === undefined
      ? {
          id: attachmentId,
          fileName: "notes.txt",
          storagePath: `tickets/${ticketId}/generated-notes.txt`,
          mimeType: "text/plain",
  ...
  const storage: StorageService = {
    ...
    download: async (path) => {
      ...
      return { body: Buffer.from("backup"), contentType: "text/plain" };
    },
    remove: async (path) => {
      storageState.removals.push(path);
    },
...
describe("attachment validation", () => {
  ...
  it("rejects an unsupported MIME type", () => {
    ...
    assert.throws(
      ...
      (error: unknown) => error instanceof HttpError && error.code === "UNSUPPORTED_FILE_TYPE",
    );
  });
...
describe("attachment service authorization and consistency", () => {
  it("allows a requester to upload to their own active ticket", async () => {
    ...
    assert.match(
      storageState.uploads[0]!,
      new RegExp(`^tickets/${ticketId}/[0-9a-f-]+-notes\\.txt$`),
    );
  });
  ...
  it("prevents a requester viewing another requester's attachment", async () => {
    ...
    assert.equal(
      await errorCode(service.createUrl(ticketId, attachmentId, otherRequester)),
  ...
  it("prevents requester removal after the ticket leaves OPEN", async () => {
    ...
    assert.equal(
      await errorCode(service.remove(ticketId, attachmentId, requester)),
      "ATTACHMENT_ACCESS_DENIED",
    );
  });
  ...
  it("rejects an unassigned technician upload", async () => {
    const { service } = harness();
    assert.equal(
      await errorCode(service.upload(ticketId, technician, [textFile()])),
      "ATTACHMENT_ACCESS_DENIED",
    );
  });
  ...
  it("distinguishes an invalid ticket from a missing attachment", async () => {
    ...
    assert.equal(
      await errorCode(invalid.service.createUrl(ticketId, attachmentId, requester)),
      "TICKET_NOT_FOUND",
    );
    const missing = harness({ attachment: null });
    assert.equal(
      await errorCode(missing.service.createUrl(ticketId, attachmentId, requester)),
  ...
  it("cleans up earlier objects when a multi-file storage upload fails", async () => {
    ...
    assert.equal(
      await errorCode(
        service.upload(ticketId, requester, [textFile("one.txt"), textFile("two.txt")]),
      ),
  ...
  it("cleans up storage when database metadata creation fails", async () => {
    ...
    assert.equal(
      await errorCode(service.upload(ticketId, requester, [textFile()])),
      "ATTACHMENT_UPLOAD_FAILED",
    );
    assert.deepEqual(storageState.removals, storageState.uploads);
  });
