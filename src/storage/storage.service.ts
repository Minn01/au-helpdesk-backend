...
export class SupabaseStorageService implements StorageService {
  ...
  async download(path: string): Promise<StoredFile> {
    ...
    if (error || !data)
      throw new Error(`Storage download failed: ${error?.message ?? "object unavailable"}`);
    return {
  ...
  async remove(path: string): Promise<void> {
    ...
    if (error || !data?.length)
      throw new Error(`Storage delete failed: ${error?.message ?? "object unavailable"}`);
  }
  ...
  async createSignedUrl(path: string, expiresInSeconds: number): Promise<string> {
    ...
    if (error || !data?.signedUrl)
      throw new Error(`Signed URL creation failed: ${error?.message ?? "object unavailable"}`);
