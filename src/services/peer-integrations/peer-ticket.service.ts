...
export class PeerTicketService {
  ...
  async createFromEduCore(input: EduCoreTicketInput) {
    ...
    if (!student)
      throw new HttpError(
        404,
        "STUDENT_NOT_FOUND",
        "No active HelpDesk requester matches this student",
      );
    ...
    if (!category)
      throw new HttpError(
        ...
        "Course Registration category is unavailable",
      );
  ...
  private async findExisting(externalEventId: string) {
    const reference = await this.database.peerTicketReference.findUnique({
      where: {
        sourceSystem_externalEventId: { sourceSystem: PeerSourceSystem.EDUCORE, externalEventId },
      },
