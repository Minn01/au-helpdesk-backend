...
describe("OpenAI ticket classification adapter", () => {
  it("uses schema-constrained Responses API output and validates it", async () => {
    ...
    assert.equal(
      (request?.text as { format: { type: string; strict: boolean } }).format.type,
      "json_schema",
    );
    assert.equal(
      (request?.text as { format: { type: string; strict: boolean } }).format.strict,
      true,
    );
  ...
  for (const [name, output] of [
    ...
    [
      ...
      JSON.stringify({ category: "Software", priority: "MEDIUM", summary: null }),
    ],
    [
      "invalid priority",
      JSON.stringify({ category: "Network", priority: "CRITICAL", summary: null }),
    ],
    [
      "oversized summary",
      JSON.stringify({ category: "Network", priority: "MEDIUM", summary: "x".repeat(241) }),
    ],
  ] as const) {
