...
export class AdminManagementService {
  ...
  async updateUser(userId: string, input: UserUpdate, actorId: string) {
    return this.database.$transaction(async (transaction) => {
      ...
      const removesTechnicianAccess =
        user.role === UserRole.TECHNICIAN &&
        (input.isActive === false ||
          (input.role !== undefined && input.role !== UserRole.TECHNICIAN));
  ...
  async dashboard() {
    ...
    const [
      ...
      this.database.ticket.count({
        where: { status: TicketStatus.OPEN, assignedTechnicianId: null },
      }),
    ...
    const technicianIds = workload.flatMap((entry) =>
      entry.assignedTechnicianId ? [entry.assignedTechnicianId] : [],
    );
    const technicians =
    ...
    return {
      ...
      ticketsByPriority: Object.fromEntries(
        byPriority.map((entry) => [entry.priority, entry._count._all]),
      ),
      technicianWorkload: workload.flatMap((entry) => {
        ...
        return [
          {
            technicianId: entry.assignedTechnicianId,
            displayName: technician?.displayName ?? "Unknown",
            isActive: technician?.isActive ?? false,
            activeTickets: entry._count._all,
          },
        ];
      }),
      recentTickets,
