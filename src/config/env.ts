...
const createEduCoreConfig = (
  ...
  if (baseUrl.protocol !== "http:" && baseUrl.protocol !== "https:")
    throw new Error("EDUCORE_BASE_URL must use HTTP or HTTPS");
  if (baseUrl.username || baseUrl.password || baseUrl.search || baseUrl.hash)
    throw new Error("EDUCORE_BASE_URL is invalid");
  if (nodeEnv === "production" && baseUrl.protocol !== "https:")
    throw new Error("EDUCORE_BASE_URL must use HTTPS in production");
  ...
  const contextPathTemplate =
    environment.EDUCORE_CONTEXT_PATH_TEMPLATE ||
  ...
  const timeoutMs =
    environment.EDUCORE_TIMEOUT_MS === undefined ? 4_000 : Number(environment.EDUCORE_TIMEOUT_MS);
...
const createMicrosoftConfig = (
  ...
  for (const key of Object.keys(values) as Array<keyof typeof values>) {
    if (!values[key])
      throw new Error(`${names[key]} is required when Microsoft authentication is configured`);
  }
  ...
  if (redirectUri.username || redirectUri.password || redirectUri.hash)
    throw new Error("MICROSOFT_REDIRECT_URI is invalid");
  if (frontendUrl.username || frontendUrl.password || frontendUrl.search || frontendUrl.hash)
    throw new Error("FRONTEND_URL is invalid");
  if (
    nodeEnv === "production" &&
    (redirectUri.protocol !== "https:" || frontendUrl.protocol !== "https:")
  ) {
  ...
  const secrets: Record<string, string | undefined> =
    secretSource === "azure-key-vault"
      ? await (
          ...
          new AzureKeyVaultSecretLoader(
            required(environment.AZURE_KEY_VAULT_URL, "AZURE_KEY_VAULT_URL"),
          )
        ).load(Object.values(KEY_VAULT_SECRET_NAMES))
      : environment;
  const value = (name: keyof typeof KEY_VAULT_SECRET_NAMES) =>
    secretSource === "azure-key-vault" ? secrets[KEY_VAULT_SECRET_NAMES[name]] : secrets[name];
