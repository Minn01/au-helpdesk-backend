...
describe("EduCore HTTP client", () => {
  it("adds x-api-key and normalizes a successful response", async () => {
    ...
    const result = await new HttpEduCoreClient(config, request).getRegistrationContext(
      "event/with spaces",
    );
  ...
  it("maps timeout/network failure without leaking the API key", async () => {
    ...
    assert.equal(
      await code(new HttpEduCoreClient(config, request).getRegistrationContext("event")),
      "EDUCORE_UNAVAILABLE",
    );
  });

  for (const [status, expected] of [
    [401, "EDUCORE_AUTH_FAILED"],
    [403, "EDUCORE_AUTH_FAILED"],
    [404, "EDUCORE_CONTEXT_NOT_FOUND"],
    [500, "EDUCORE_UNAVAILABLE"],
  ] as const) {
    it(`maps upstream ${status}`, async () => {
      const request: typeof fetch = async () =>
        new Response("upstream body must not escape", { status });
      assert.equal(
        await code(new HttpEduCoreClient(config, request).getRegistrationContext("event")),
        expected,
      );
