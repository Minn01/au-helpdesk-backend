...
const users = {
  findById: async (id: string) => (id === admin.id ? admin : id === student.id ? student : null),
};
    ...
    server = await new Promise<Server>((resolve) => {
      const candidate = createApp({
        ...
        adminTickets: {
          ...
          listComments: async () => [],
          assign: async (_ticketId, technicianId) => {
            assignedTo = technicianId;
            return { id: _ticketId } as never;
          },
        },
        adminManagement: {
          dashboard: async () => ({ totalTickets: 0 }),
          listCategories: async () => [],
          createCategory: async (input: { name: string; description: string | null }) =>
            ({ id: "20000000-0000-4000-8000-000000000001", ...input }) as never,
